...
public class Paths {
	 ...
	 * does not exist.
	 * Note: The empty list is NOT "null"; it is a list with 0 elements. */
	public static List<Node> shortPath(Node start, Node end) {
		/* TODO Read note A7 FAQs on the course piazza for ALL details. */
		Heap<Node> F= new Heap<Node>(); // As in lecture slides
		...
		int hosnum=0; //number of potential hostile planets
		HashMap<Node, SFdata> map= new HashMap<Node, SFdata>();
		...
		while (F.size() != 0) {
			...
			// if the closest planet is hostile,  then increase the number of 
			//potential hostile planets
			if (f.isHostile()) { hosnum++; } 
			...
			for (Edge e : f.getExits()) {// for each neighbor w of f
				...
				if (wData == null ) { //if w not in S or F
					//don't add the neighbor to frontier set if 
					//the neighbor is hostile and we have already seen two hostile planets.
					if (!w.isHostile() ||  (hosnum ==2)){ 
						map.put(w, new SFdata(newWdist, f));
						F.add(w, newWdist);	
